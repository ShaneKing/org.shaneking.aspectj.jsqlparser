...
public class TableNamesFinder implements SelectVisitor, FromItemVisitor, ExpressionVisitor, ItemsListVisitor, SelectItemVisitor, StatementVisitor {
   ...
   */
  public Set<Tuple.Pair<String, String>> findTableSet(Expression expr) {
    init(true);
    expr.accept(this);
    return tables;
  }
...
//      && !tables.contains(tableWholeName)) {
      tables.add(Tuple.of(tableWholeName.toLowerCase(), tableName.getAlias() == null ? null : Strings.nullToEmpty(tableName.getAlias().getName()).toLowerCase()));
    }
  }

 ...
 */
package org.shaneking.jsqlparser.util;
...
import org.shaneking.jsqlparser.annotation.SensitiveItemsFinderAlias;
import org.shaneking.jsqlparser.annotation.SensitiveItemsFinderAsterisk;
import org.shaneking.jsqlparser.annotation.SensitiveItemsFinderPath;
import org.shaneking.jsqlparser.annotation.SensitiveItemsFinderTransformed;
...
public class SensitiveItemsFinder implements SelectVisitor, FromItemVisitor, ExpressionVisitor, ItemsListVisitor, SelectItemVisitor, StatementVisitor {
  ...
  public Tuple.Pair<Set<String>, Map<String, Set<Tuple.Quadruple<String, String, Set<String>, Boolean>>>> mergeAliasTableMap(Map<String, Tuple.Pair<Set<String>, Map<String, Set<Tuple.Quadruple<String, String, Set<String>, Boolean>>>>> columns) {
    return mergeAliasTableTuplePairCollection(columns.values());
  }

  public Tuple.Pair<Set<String>, Map<String, Set<Tuple.Quadruple<String, String, Set<String>, Boolean>>>> mergeAliasTableTuplePairCollection(@NonNull Collection<Tuple.Pair<Set<String>, Map<String, Set<Tuple.Quadruple<String, String, Set<String>, Boolean>>>>> collection) {
  ...
  private void processAllColumns(String alias) {
    ...
    aliasTableName = aliasTableName.toLowerCase();
  ...
  private void processWithItemsList(@NonNull List<WithItem> withItemsList) {
    ...
    for (WithItem withItem : withItemsList) {
      ...
      withItemMap.put(withItem.getName().toLowerCase(), mergeAliasTableMap(itemMap));
    }
...
//    }
  ...
  public void visit(Table tableName) {
    String realTableName = extractTableName(tableName).toLowerCase();
  ...
  public void visit(SubSelect subSelect) {
    ...
    //2
    String aliasTableName = (subSelect.getAlias() != null && subSelect.getAlias().getName() != null) ? subSelect.getAlias().getName().toLowerCase() : null;
    if (Strings.isNullOrEmpty(aliasTableName)) {
  ...
  public void visit(Column tableColumn) {
    ...
    if (columnSet == null) {
      ...
      if (columnSet == null) {
        newColumnSet = Tuple.getFirst(aliasTableTuple).stream().map(realTable -> Tuple.of(realTable, tableColumn.getColumnName().toLowerCase(), addPath(Sets.newHashSet()), transformed)).collect(Collectors.toSet());
      } else {
        newColumnSet = columnSet.stream().map(tupleQuadruple -> Tuple.of(Tuple.getFirst(tupleQuadruple), tableColumn.getColumnName().toLowerCase(), addPath(Sets.newHashSet()), transformed)).collect(Collectors.toSet());
      }
    } else {
      //columnName is aliasColumnName
...
//    }
  ...
  public void visit(SelectExpressionItem item) {
    String handleSelectExpressionItemAlias = selectExpressionItemAlias;
    selectExpressionItemAlias = (item.getAlias() == null ? String0.EMPTY : (item.getAlias().getName() == null ? String0.EMPTY : item.getAlias().getName().toLowerCase()));
...
//    processFromItem(parenthesis.getFromItem(), parenthesis.getAlias());
  ...
  public void visit(DescribeStatement describe) {
    throw new UnsupportedOperationException(Joiner.on(String0.COLON).join(NOT_SUPPORTED_STATEMENT_TYPE_YET, describe));
//    describe.getTable().accept(this);
  }

  @Override
  public void visit(ExplainStatement explain) {
    throw new UnsupportedOperationException(Joiner.on(String0.COLON).join(NOT_SUPPORTED_STATEMENT_TYPE_YET, explain));
//    explain.getStatement().accept(this);
  }

  @Override
  public void visit(NextValExpression nextVal) {
    throw new UnsupportedOperationException(Joiner.on(String0.COLON).join(NOT_SUPPORTED_EXPRESSION_YET, nextVal));
  }
